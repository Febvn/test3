...
Penjelasan singkat:
- Komponen `Tooltip` membuat daftar icon sosial yang diberi tooltip muncul saat hover.
- `StyledWrapper` berisi semua style untuk layout daftar icon dan tooltip (posisi absolute, transisi, dsb).
- Ikon menggunakan SVG inline sehingga mudah dikustom warna via CSS.
- `data-social` digunakan untuk memberi warna hover yang berbeda per platform.
