...
Penjelasan (ringkasan per blok):
- Import React dan `styled-components`.
- Definisi styled components: CardContainer, CardImage, CardContent, CardTitle, dll. Setiap styled component mengatur tampilan visual kartu.
- `ArticleCard` component: menerima prop `article`, mem-format tanggal, menampilkan gambar (jika ada), judul, deskripsi, sumber, tanggal, dan link "Read More".

Catatan: beberapa properti warna memiliki hex channel alpha yang redundant (mis. `#ffffffff`) — tidak berbahaya tapi bisa dirapikan.
